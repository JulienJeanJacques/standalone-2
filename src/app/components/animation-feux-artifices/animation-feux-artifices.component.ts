...
//     this.musicService.playMusic('forGoodResponse-3.mp3', 10, 20);
//   }
...
// }
// }
export class AnimationFeuxArtificesComponent implements OnInit, OnDestroy {
  particles: { x: number; y: number; color: string; size: string; opacity: number; angle: number; speed: number; }[] = [];
  private COLORS = ['#FF6347', '#FFD700', '#ADFF2F', '#00BFFF', '#FF69B4', '#FFA500', '#40E0D0', '#FF4500', '#DA70D6', '#7CFC00', '#FF1493', '#FF8C00'];
  
  private intervalId!: any; // référence pour clearInterval
  ...
  startFireworksLoop() {
    const intervalTime = 1500; 
    let totalDuration = 18000; // animation totale : 10 secondes
  ...
  launchFirework(offsetX: number, baseY: number) {
    ...
    for (let i = 0; i < explosionCount; i++) {
      const angle = Math.random() * Math.PI * 2 * 1.8 - Math.PI * 0.9; 
